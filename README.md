## Hi there I'm Xiaotong!👋

👨‍💻 **南开大学计算机科学与技术** | 大三在读

---

- 💡 数据分析：熟练使用SQL/Python进行数据抓取、清洗、分析与可视化，具备数据驱动问题解决能力。
- 🛠️ 产品核心技能：熟练使用Axure/Figma/墨刀制作高保真原型，撰写PRD文档，撰写竞品分析文档，掌握产品从0到1的设计流程。
- 🌱 大模型应用：熟悉大模型基本原理与应用场景，有基于大模型的产品项目落地经验（鸿蒙AI音乐APP）。
- 📦 通用技能：熟练使用Office办公软件，CET-6 581分，具备良好的英文读写能力。
- 😄 软技能：优秀的跨团队沟通协作能力、项目统筹能力与抗压能力，具备主动发现问题、解决问题的意识，快速学习能力与适应能力。
- 🌏 热爱生活，喜欢旅行、摄影、写作和厨艺

---

📬 **Contact me**  
- 邮箱：liuyuetong905@163.com

# [![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&pause=1000&width=435&lines=Hi%F0%9F%91%8B%2C+I'm+LiuYuetong!;Welcome+to+my+homepage!)](https://github.com/xiaotong-jpg)

<!--
**xiaotong-jpg/xiaotong-jpg** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...

- ⚡ Fun fact: ...
-->
